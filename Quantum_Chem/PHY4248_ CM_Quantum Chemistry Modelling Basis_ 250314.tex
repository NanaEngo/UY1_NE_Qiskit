...
\subsection{Influence du type de couche}
Les trois méthodes HF communément utilisées sont (voir la \autoref{fig:hforb}) :

\begin{figure}
\centering
\includegraphics[width=0.5\linewidth]{Graphics/HF_Orb}
\caption{Méthodes HF suivant le type de couche.}
\label{fig:hforb}
\end{figure}

\begin{itemize}
\item \textbf{RHF (Restricted HF)} utiliśee pour des molécules à couches pleines ou fermées. Les spin-orbitales sont soit $\alpha$, soit $\beta$ et tous les orbitales sont doublement occupés par des spin-orbitales $\alpha$ et $\beta$;
 
\item \textbf{ROHF (Restricted Open-Shell HF)} utilisée pour des molécules à couches ouvertes où le nombre d'électrons les orbitales n'est pas le même. ROHF utilise autant que possible les orbitales doublement occupées et les orbitales une fois occupées par les électrons non-apariés;

\item \textbf{UHF (Unrestricted Hartree-Fock)} utilisée pour des molécules à couches ouvertes où le nombre d'électrons les orbitales n'est pas le même. Les orbitales UHF peuvent avoir avoir des spin $\alpha$ ou $\beta$, mais les orbitales $\alpha$ et $\beta$ peuvent avoir des composants spatiales différents.
\end{itemize}
