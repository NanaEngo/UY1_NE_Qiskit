...
\item $\mathtt{U}_{en}= -\sum_{i,I}\frac{e^2}{4\pi\epsilon_0}\frac{Z_I}{|\mathbf{r}_i-\mathbf{R}_I|}$ est l'énergie potentielle d'attraction Coulombienne entre les électrons et les noyaux;
\item $\mathtt{U}_{ee}=+\frac{1}{2}\sum_{i\neq j}\frac{e^2}{4\pi\epsilon_0}\frac{1}{|\mathbf{r}_i-\mathbf{r}_j|}$ est l'énergie potentielle de répulsion Coulombienne entre les électrons;
\item $\mathtt{U}_{nn}=+\frac{1}{2}\sum_{I\neq J}\frac{e^2}{4\pi\epsilon_0}\frac{Z_IZ_J}{|\mathbf{R}_I-\mathbf{R}_J|}$ est l'énergie potentielle de répulsion Coulombienne entre les noyaux.
\end{itemize}
...
Bien que le nombre de spin-orbitales considérées, $M$, soit généralement plus élevé que le nombre d'électrons dans le système, $N$, les électrons ne peuvent occuper que $N$ des spin-orbitales dans un déterminant Slater donné. En conséquence, le déterminant de Slater ne contient que les spin-orbitales occupées $N$.

Dans la base des \textbf{orbitales moléculaires (MO)}, chaque spin-orbitale $\phi_i(\mathbf{r})$ est une combinaison linéaire des \textbf{orbitales atomiques (AO)} via la méthode \textbf{LCAO} (Linear Combination of Atomic Orbitals):
\begin{equation}
\phi_i(\mathbf{r}) = \sum_{\mu} C_{\mu i} \chi_\mu(\mathbf{r} - \mathbf{R}_A),
\end{equation}
où $\chi_{\mu}(\mathbf{r})$ sont les fonctions de base  atomiques (e.g., fonctions de Slater ou Gaussiennes) et $C_{\mu i}$ les coefficients d’expansion à optimiser.
...
\begin{enumerate}
\item \textbf{Initialisation} des orbitales moléculaires avec une estimation $\vb{C}^{(0)}$. On founit les coordonnées 3D des atomes du système moléculaire.
\item \textbf{Construction de l'opérateur de Fock $\vb{F}$.} On calculs les matrices de Coulomb ($\vb{J}$) et d'échange ($\vb{K}$) (elles sont très souvent coûteuses).
...
\item \textbf{Vérification du critère de convergence.} On comparer l'énergie $E^{(k+1)}$ avec $E^{(k)}$. Si $\big|E^{(k+1)} - E^{(k)}\big| < \delta$ (le seui), le processus est terminé.
...
\caption{\textbf{Orbitales moléculaires et approches Hartree-Fock.} Choix de la méthode Hartree-Fock (RHF, ROHF, UHF) en fonction de l'occupation des orbitales moléculaires. RHF est adapté aux systèmes à couches fermées, ROHF aux couches ouvertes, et UHF aux systèmes fortement magnétiques, mais au prix d'une complexité accrue et d'un risque de spin contamination.}
\label{fig:hforb}
\end{figure}
...
\begin{enumerate}
    ...
    \item {Construction du déterminant de Slater.} On construit une fonction d'état multi-électronique antisymétrique en utilisant un déterminant de Slater.
    \item {Méthode de Hartree-Fock (SCF).} On utilise une procédure itérative pour optimiser les orbitales moléculaires en considérant que chaque électron se déplace dans un champ moyen créé par les autres.
\end{enumerate}
